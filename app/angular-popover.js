(function(angular){
	...
	var app = angular.module('angularPopover', []);
	app.directive('angularPopover', function() {
		return {
			...
			link: function($scope, element, attrs, ctrl) {
				...
				var target = document.querySelector('.angular-popover[trigger="'+$scope.trigger+'"]');

				// Add click event listener to trigger
				trigger.addEventListener('click', function(ev){
					var left, top;
					var trigger = this; //get trigger element
					var target =  document.querySelector('.angular-popover[trigger="'+$scope.trigger+'"]'); //get triger's target popover
					ev.preventDefault();
			...
			controller: ['$scope', function($scope){
				...
				var bodyListenerLogic = function(e){
					...
					do {
						if(clickedElement != document && (clickedElement.classList && (clickedElement.classList.contains('angular-popover') || clickedElement.classList.contains('angular-popover-trigger')))) {
							insidePopover = true;
							break;
						}
			...
			template: '<div class="angular-popover hide"><div class="angular-popover-wrapper {{dropDirection}}"><div class="angular-popover-content" ng-class="popoverClass"><ng-transclude></ng-transclude></div></div></div>'
		}
	});
	...
	app.factory('ngPopoverFactory', function(){
		return {
			closePopover : function(trigger){
				document.querySelector('.angular-popover[trigger='+trigger+']').classList.add('hide');
			},
			closeAll : function(){
				var allPopovers = document.querySelectorAll('.angular-popover');
	...
	var hideAllPopovers = function(trigger){
		...
		var allPopovers = trigger != undefined ? document.querySelectorAll('.angular-popover:not([trigger="'+triggerId+'"])') : document.querySelectorAll('.angular-popover');
		for(var i =0; i<allPopovers.length; i++){
